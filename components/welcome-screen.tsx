...
export default function WelcomeScreen({ language, onSuggestionClick }: WelcomeScreenProps) {
  // Update the welcome title and description to use "Onnesha" instead of "Wiser"
  const translations = {
    en: {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full space-y-6 md:space-y-8 p-3 md:p-4 max-w-2xl mx-auto">
      <div className="text-center space-y-2 md:space-y-3">
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-primary">{t.welcome}</h1>
        <p className="text-sm md:text-base text-muted-foreground">{t.description}</p>
      </div>

      <div className="w-full space-y-3 md:space-y-4">
        <p className="text-xs md:text-sm font-medium text-center">{t.suggestions}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-3">
          {suggestions.map((suggestion, index) => (
            <Card
              key={index}
              className={cn(
                "p-2 md:p-3 hover:bg-accent transition-colors cursor-pointer floating-element",
                suggestion.color,
              )}
            ...
            >
              <div className="flex items-center gap-1.5 md:gap-2">
                {suggestion.icon}
                <span className="text-xs md:text-sm">{suggestion.text}</span>
              </div>
            </Card>
          ))}
        </div>
